...
flux_bgc <- flux %>%
  left_join(ingrowth_cores, by = c("plot", "date")) %>%
  left_join(lysimeter_surface, by = c("plot", "date")) %>%
  left_join(pwchemistry, by = c("plot", "date")) %>%
  left_join(resin, by = c("plot", "date"), relationship = "many-to-many") %>%
  left_join(nutrient_cores, by = c("plot", "date")) %>%
  clean_names()
